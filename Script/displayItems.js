function displayItems(filter_id)
{
	...
	fetch("../Json/myProducts.json").then((response)=>
		response.json()
		).then((myObject)=>
		{
		for(let k in myObject)
		{
			if(myObject[k].filter_id===filter_id)
			{
					...
					for(let i in myBlocks)
					{
						...
						myBlocks[i].style.paddingTop="10px"
						myBlocks[i].style.textAlign="center"
						myBlocks[i].style.borderRadius="3%"
						myBlocks[i].style.border="2px solid black"
						myBlocks[i].style.marginTop="13px"
						myBlocks[i].style.backgroundColor="antiquewhite"
						}						
					}
					...
					if(!(String(myCartButton[j].innerHTML)==="undefined")&&(typeof(myCartButton[j])==="object"))
						...
						myCartButton[j].style.color="black"
						myCartButton[j].style.backgroundColor="mediumspringgreen"
...
function displayProduct(product)
{
	...
	<span>₹${product.price}</span><button class="float-right btn btn-info" onclick="addToCart(${product.product_id},'${product.product_name}','${product.imgSrc}',${product.price})">Shop <i class="bi bi-bag-check-fill"></i></button></div>`
}
...
function addToCart(product_id,product_name,imgSrc,price)
{
	...
	let myImgArray=localStorage.getItem("imgSrcs").split(",")
	myImgArray.push(imgSrc)
	localStorage.setItem("imgSrcs",myImgArray)
...
function proceedPayment(total)
{
	...
	let cardDiv=`<img src="../Images/Bitcoin.webp"><img src="../Images/Visa.png"><img src="../Images/Gpay.jpg"><img src="../Images/Paytm.png">`
